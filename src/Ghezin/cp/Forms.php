...
use Ghezin\cp\Core;
use Ghezin\cp\Utils;
use Ghezin\cp\party\{PartyManager, Party};
...
class Forms{
	...
	public function manageInviteForm(Player $player):void{
		...
		$form->setTitle("Manage Invitation");
	...
	public function partyDuelForm(Player $player):void{
		...
		$modes=["NoDebuff", "Gapple", "Soup", "BuildUHC"];
		$form->setTitle("Setup");
		$form->addDropdown("Choose a mode", $modes, 0);
	...
	public function partyMembersForm(Player $player):void{
		...
		$members=count($party->getMembers());
		$capacity=$party->getCapacity();
		$form->setTitle("Members (".$members."/".$capacity.")");
