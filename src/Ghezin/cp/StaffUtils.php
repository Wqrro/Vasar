...
class StaffUtils{
	...
	public function vanish(Player $player, $bool=false){
		if($bool===true){
			...
			Core::getInstance()->getServer()->broadcastMessage("§4- §c".$player->getDisplayName());
			...
			Core::getInstance()->getServer()->broadcastMessage("§2+ §a".$player->getDisplayName());
	...
	public function sendStaffAlert($type){
		switch($type){
			case "autoclick":
			$message="§8[STAFF] §4{name} flagged for AUTO-CLICK. §7({details} cps)";
			return $message;
			break;
			case "ping":
			$message="§8[STAFF] §4{name} kicked for a long period of HIGH PING. §7({details}ms)";
			return $message;
			break;
			case "reach":
			$message="§8[STAFF] §4{name} flagged for REACH. §7({details} blocks)";
			return $message;
			break;
			case "highjump":
			$message="§8[STAFF] §4{name} flagged for HIGHJUMP.";
			return $message;
			break;
			case "killaura":
			$message="§8[STAFF] §4{name} flagged for KILLAURA.";
			return $message;
			break;
			default:
			return;
	...
	public function staffPortalForm(Player $player):void{
		...
		//$form->addButton("View Player Info", -1, "", "viewinfo");
		$player->sendForm($form);
	}
	...
	public function permanentBanForm(Player $player):void{
		...
		$form->setTitle("Permanently ban ".$this->targetPlayer[Utils::getPlayerName($player)]);
	...
	public function temporaryBanForm(Player $player):void{
		...
		$form->setTitle("Action for ".$this->targetPlayer[Utils::getPlayerName($player)]);
	...
	public function searchedPermanentBanForm(Player $player, $searchedtarget):void{
		...
		$form->setTitle("Permanently ban ".$this->searchedtarget);
		$form->addButton("Confirm", -1, "", "confirm");
		$form->addButton("« Back", -1, "", "exit");
		$player->sendForm($form);
	}
	public function searchedTemporaryBanForm(Player $player, $searchedtarget):void{
		...
		$form->setTitle("Action for ".$this->searchedtarget);
		$form->addButton("Unfair Advantage", -1, "", "unfairadvantage");
		$form->addButton("Punishment Evasion", -1, "", "evasion");
		$form->addButton("Excessive Toxicity", -1, "", "excessivetoxicity");
		$form->addButton("Exploitation", -1, "", "exploitation");
