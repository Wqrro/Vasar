...
use Ghezin\cp\arenas\DuelArena;
use Ghezin\cp\duels\groups\DuelGroup;
use Ghezin\cp\duels\groups\PartyDuelGroup;
use Ghezin\cp\duels\groups\BotDuelGroup;
use Ghezin\cp\duels\groups\MatchedGroup;
use Ghezin\cp\duels\groups\MatchedBotGroup;
use Ghezin\cp\duels\groups\QueuedPlayer;
use Ghezin\cp\Core;
use Ghezin\cp\Utils;
use Ghezin\cp\Kits;
use Ghezin\cp\party\Party;
	...
	public function startDuel(MatchedGroup $group):void{
		...
		if(!is_null($arena) and $this->isValidMatched($group)){
			...
			if($group->isPlayerOnline() and $group->isOpponentOnline()){
				...
				/*
				$p->teleport(new Location(0, 0, 0, 90, 0, $duellevel));
				$o->teleport(new Location(0, 0, 0, 90, 0, $duellevel));
				*/
			}
