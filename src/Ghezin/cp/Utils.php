...
use Ghezin\cp\Kits;
use Ghezin\cp\entities\Hook;
use Ghezin\cp\VasarChunkLoader;
use Ghezin\cp\forms\{SimpleForm, CustomForm, ModalForm};
use Ghezin\cp\bots\{EasyBot, MediumBot, HardBot, HackerBot};
...
class Utils{
	...
	public static function formatElo($elo){
		$format="§8".$elo;
		if($elo>=0){
			$format="§8".$elo;
		}
	...
	public static function customPots(Item $potion, Player $player, bool $animate=false){
		...
		//$potion->onClickAir($player, $player->getDirectionVector()->add(0, -0.4, 0));
		//$potion->onClickAir($player, $player->getDirectionVector()->subtract(0, 0.3, 0));
	...
	public static function offerVoteRewards($player){
		...
		if($onlineplayer instanceof Player){
			...
			//Core::getInstance()->getServer()->broadcastMessage("§b".$onlineplayer->getName()." §3voted at §b".Core::getInstance()->getVote()." §3and claimed their rewards!");
	...
	public static function throwItem(Item $item, $player, bool $animate=false){
		...
		$dx=$dir->getX();
		$dz=$dir->getZ();
		//$item->onClickAir($player, $player->getDirectionVector()->add($dx - 1, 2, $dz - 1));
	...
	public static function matchOutcome($player, int $reason, bool $isRanked=false){
		switch($reason){
			...
			if($isRanked===true){
				...
				if(!is_null($oplayer)) $oplayer->sendMessage("§fElo Earned: §a+".$setelo);
			}
			break;
			case 1:
			$oplayer=self::getPlayer($player);
			...
			if($isRanked===true){
				...
				if(!is_null($oplayer)) $oplayer->sendMessage("§fElo Lost: §c-".$setelo);
			}
	...
	public static function postWebhook(String $url, String $content, String $replyTo='Vasar'){
	...
	public static function getChatFormat($rank){
		switch($rank){
			case "Player":
			$format="§7{clan}§r§f{name}§7: {message}";
			return $format;
			break;
			case "Voter":
			$format="§7{clan}§r§f[§6Voter§f] {name}: §6{message}";
			return $format;
			break;
			case "Elite":
			$format="§7{clan}§r§f[§aElite§f] {name}§f: §a{message}";
			return $format;
			break;
			case "Premium":
			$format="§7{clan}§r§f[§bPremium§f] {name}§f: §b{message}";
			return $format;
			break;
			case "Booster":
			$format="§7{clan}§r§f[§5Booster§f] {name}§f: §5{message}";
			return $format;
			break;
			case "YouTube":
			$format="§7{clan}§r§f[§cYou§fTube] {name}§f: §c{message}";
			return $format;
			break;
			case "Famous":
			$format="§7{clan}§r§f[§d§oFamous§r§f] {name}§f: §d{message}";
			return $format;
			break;
			case "Trainee":
			$format="§7{clan}§r§f§a[Trainee] {name}§f: {message}";
			return $format;
			break;
			case "Helper":
			$format="§7{clan}§r§f§5[Helper] {name}§f: {message}";
			return $format;
			break;
			case "Mod":
			$format="§7{clan}§r§f§9[Mod] {name}§f: {message}";
			return $format;
			break;
			case "HeadMod":
			$format="§7{clan}§r§f§3[Head-Mod] {name}§f: {message}";
			return $format;
			break;
			case "Admin":
			$format="§7{clan}§r§f§6[Admin] {name}§f: {message}";
			return $format;
			break;
			case "Manager":
			$format="§7{clan}§r§f§c[Manager] {name}§f: {message}";
			return $format;
			break;
			case "Owner":
			$format="§7{clan}§r§f§4[Owner] {name}§f: {message}";
			return $format;
			break;
			default:
			$format="§7{clan}§r§f§7{name}§f: {message}";
			return $format;
			break;
		}
	}
