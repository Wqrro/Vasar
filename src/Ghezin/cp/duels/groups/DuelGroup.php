...
use Ghezin\cp\duels\groups\DuelSpectator;
use Ghezin\cp\tasks\onetime\OpenArenaTask;
use Ghezin\cp\tasks\onetime\LoadLevelTask;
use Ghezin\cp\forms\{SimpleForm, ModalForm, CustomForm};
...
class DuelGroup{
		...
		if($this->isLoadingDuel()){
			...
			if($this->countdownTick % 20 === 0 and $this->countdownTick !== 0){
				...
				if($second !== 0){
					...
					$this->broadcastTitle("§bDuel!", "The match has started", 5, 10, 7);
				}
	...
	private function endDuel(bool $endPrematurely=false, bool $disablePlugin=false, $win=false):void{
		...
		if($win===true){
			...
			/*$pc='%';
			$pform=new SimpleForm(function(Player $player, $data=null):void{});
			$pform->setTitle("Post Match Details");
			$pform->setContent("§bYou\n§fHealth: ".$finalhealthP."\nHits: ".count($this->playerHits)."\nAccuracy: ".$accuracyP.$pc."\nPots: ".$ppots."\n\n§b".Utils::getPlayerDisplayName($opponent)."\n§fHealth: ".$finalhealthO."\nHits: ".count($this->oppHits)."\nAccuracy: ".$accuracyO."%\nPots: ".$opots);
			$pform->addButton("Exit");
			$player->sendForm($pform);
			$oform=new SimpleForm(function(Player $opponent, $data=null){});
			$oform->setTitle("Post Match Details");
			$oform->setContent("§bYou\n§fHealth: ".$finalhealthO."\nHits: ".count($this->oppHits)."\nAccuracy: ".$accuracyO.$pc."\nPots: ".$opots."\n\n§b".Utils::getPlayerDisplayName($player)."\n§fHealth: ".$finalhealthP."\nHits: ".count($this->playerHits)."\nAccuracy: ".$accuracyP."%\nPots: ".$ppots);
			$oform->addButton("Exit");
			$opponent->sendForm($oform);
			
			
			$player->sendMessage("§3Match Overview");
			$player->sendMessage("§7You");
			$player->sendMessage("Health: ".$finalhealthP);
			$player->sendMessage("Pots: ".$ppots);
			$player->sendMessage(" ");
			$player->sendMessage("§7".Utils::getPlayerDisplayName($opponent));
			$player->sendMessage("Health: ".$finalhealthO);
			$player->sendMessage("Pots: ".$opots);
			
			$opponent->sendMessage("§3Match Overview");
			$opponent->sendMessage("§7You");
			$opponent->sendMessage("Health: ".$finalhealthO);
			$opponent->sendMessage("Pots: ".$opots);
			$opponent->sendMessage(" ");
			$opponent->sendMessage("§7".Utils::getPlayerDisplayName($player));
			$opponent->sendMessage("Health: ".$finalhealthP);
			$opponent->sendMessage("Pots: ".$ppots);*/
		...
		/*
		if(Server::getInstance()->isLevelLoaded($this->arena->getLevel()->getName())) Server::getInstance()->unloadLevel($this->arena->getLevel(), true);
		if(!Server::getInstance()->isLevelLoaded($this->arena->getLevel()->getName())) Server::getInstance()->loadLevel($this->arena->getLevel()->getName());
		
		Core::getInstance()->getScheduler()->scheduleDelayedTask(new LoadLevelTask(Core::getInstance(), $this->arenaName), 20*5);
		
		Core::getInstance()->getScheduler()->scheduleDelayedTask(new OpenArenaTask(Core::getInstance(), $this->arenaName), 20*10);
		*/
	}
